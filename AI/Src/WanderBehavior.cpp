...
X::Math::Vector2 AI::WanderingBehavior::Calculate(Agent& agent)
{
	//apply random jitter
	...
	if (IsDebug())
	{
		X::Math::Vector2 wanderCenter = X::Math::TransformCoord({ 0.0f, mWanderDistance }, worldTransform);
...
void AI::WanderingBehavior::SetUp(float radius, float distance, float jitter)
{
